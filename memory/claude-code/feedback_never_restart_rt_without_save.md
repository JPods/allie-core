---
name: Never restart Route-Time without confirming save
description: Server restart destroys in-memory network; cost Bill 1 hour of Greenville editing on 2026-07-05
type: feedback
---

NEVER restart the Route-Time server without confirming Bill has saved his work AND verifying the save captured his actual network. The curl backup trick only saves what the server holds, which may differ from the browser if the server was restarted mid-session.
