...
- whether this lesson belongs only to SketchUp, or also applies to Route-Time or the physical pods

### Key Design Invariants

These are non-negotiable and come from JPods physics and hard-won experience:

1. **CCW guideway direction** — guideways are one-way, counter-clockwise when viewed from above. This applies to the SketchUp model, Route-Time, and the physical track equally. Drawing a guideway backwards is not detectable by looking at it — only the direction tag distinguishes inbound from outbound.
...
| `jpod_network.rb` | Reads `network_definition` from `followme.json`; builds dual guideways; adaptive Bezier sampling |
...
| `nora.rb` | `note_repeated_struggle()`, `clear_struggle()`, standalone JSON observation log (`*.nora-log.json`) |
...
**Note on Route-Time relationship:** Route-Time uses a parallel Python graph (`engine/network.py`), not the `followme.json` directly. The two graphs should agree on topology. When they disagree, the physical behavior is the arbiter.

### `network_definition` in `followme.json`

The Build button reads the `network_definition` block embedded inside `<model>.followme.json`:
...
Edit via the Network Editor panel (`Network › Network Editor & Inspect`) or directly in `<model>.followme.json`.
...
2. Flag cross-domain implications when they arise (SketchUp decision → Route-Time consequence; SketchUp convention → physical deployment consequence)
...
5. Create or update the corresponding WebClerk action, note, or WhatIf record if follow-up remains
...
The rule: long-form explanation belongs in the readmes; structured follow-up belongs in WebClerk; model truth belongs in the SketchUp model and its export.
...
| Stop and Review (3 consecutive) | No formal equivalent yet | Nora `stop_and_review` JSON entry (`*.nora-log.json`) | Repeated identical failure is a signal, not bad luck |
...
| `network_definition` in `followme.json` | CP connection in `api.py` state | Physical guideway installation between switches | Declared topology drives all downstream agents |
...
- `network_definition` block inside `followme.json`: format and stub index logic
...
|------|----------|-----------|
| 2026-04-27 | Allie is always present in SketchUp sessions — not a start/end consultant | Pattern recognition and cross-domain warnings have maximum value in real time, not after the fact |
...
| 2026-04-27 | WebClerk is the structured operating database for SketchUp follow-up and coordination | Without it, accountable work exists only as conversational residue |
