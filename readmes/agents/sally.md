...
| 2026-06-13 | trip.json = complete route, gw_platform to gw_platform | Natalie issues trips that include gw_platform by track name — correct and expected. The trip.json written at dispatch must show the full circuit (departure platform → loop × N → return tracks → gw_platform) so any reader of the log sees the complete route without guessing what happens inside the station. Sally and Nora clip the gw_platform tracks at runtime: DEPARTURE — clip_start trims gw_platform to the mm from the pod's slot to the platform exit; ARRIVAL — park maneuver trims gw_platform to the mm from platform entry to the assigned slot. The log shows the contract; execution clips to the actual distance. |
