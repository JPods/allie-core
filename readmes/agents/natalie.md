...
| 2026-06-18 | **Build embeds world-space track geometry from lines.computed.json into network.json; Natalie confirms at animation start** | lines.computed.json lives at template level in local frame coordinates. During Build, each station's world transform (translation_mm + rotation_deg from network.json) is applied to template-local pts_mm and the result is embedded into the natalie section of network.json. At animation start, Natalie reads lines.computed.json from the template and compares its generated_at timestamp to network.json's build timestamp — if Compute is newer than Build, Natalie refuses to animate and reports "Run Build before animating." This enforces Compute→Build→Animate as the required sequence and eliminates the proximity fallback path entirely. Requiring Build after moving stations is correct: world coordinates changed and network.json is stale until rebuilt. Decided after diagnosing 130 proximity fallbacks caused by Natalie reading path.json (raw geometry, no direction) instead of computed track direction data. |
...
lines.computed.json is template-local (local frame, Noelle-generated). Build transforms those coordinates to world space using each station's transform and embeds them in network.json.natalie. Natalie at animation start reads lines.computed.json from the template, checks its generated_at against network.json's build timestamp, and refuses to animate if Compute postdates Build. This makes "Compute → Build → Animate" the enforced sequence. The proximity fallback (130 per trip, 4 reversals) was the symptom of Natalie reading path.json (raw geometry) with no direction/successor data at all. The confirmation is not a fallback — it is a validator that stops animation before silent degradation starts.
*Provenance: 2026-06-18 team review of 2_thru_dip fresh animation log; Bill, Claude Code, Allie, Sally, Natalie.*
