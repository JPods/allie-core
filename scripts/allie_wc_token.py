...
Reads credentials from /Users/williamjames/Allie/config/wc_credentials.json (not profile.json —
...
Usage:
  ...
  # As env var:
  export WC_TOKEN=$(python3 /Users/williamjames/Allie/scripts/allie_wc_token.py)
...
def fetch_token(email: str, password: str) -> dict:
    ...
    # /wcapi/login/ returns {"message": "login successful", "data": {"access": ...}}
    # on failure returns {"error": "..."} or {"message": "..."}
    if "data" not in result or "access" not in result.get("data", {}):
...
def main():
    ...
    parser.add_argument("--agent", default="allie", choices=["allie", "athena", "alice"],
...
def run_setup():
    ...
    for agent, defaults in [
        ...
        ("alice",  {"email": "alice@jpods.com",  "wc_id": 50,  "connection_id": 24}),
    ]:
